...
public class HeapCiudad {
    ...
    public HeapCiudad(int cantCiudades) {
        ...
        comparator = comparator.thenComparing(Comparator.comparing(Ciudad :: nombre).reversed());   // O(1)


        arrayCiudad = new Ciudad[cantCiudades];                                                     // O(1)
        heapPorSuperavit = new ArrayList<>(cantCiudades);                                           // O(1)
        for(int i = 0; i < cantCiudades; i++) {                                                     // C iteraciones totales.
       ...
       // El for loop cuesta O(c).
        ...
        // Nota: no hace falta ordenarlo pues todas tienen ganancias y perdidas de 0 al inicio y 
        // solo tras despacharlas las tenemos que modificar.
    ...
    // 'modificar' cuesta log(n).
    private void modificar(int index) {                                                     // O(log n)
        if (index == 0) {                                                                   // O(1)
            bajarSuperavit(index);                                                          // O(log n)
        } else if (prioridadDeAlgunHijoEsMayorSuperavit(index)) {                           // O(1)
            bajarSuperavit(index);                                                          // O(log n)
        } else {
            subirElementoSuperavit(index);                                                  // O(log n)
        }
    }
    ...
    private boolean esHoja(int indice) {
        return indice * 2 + 1 >= this.arrayCiudad.length;                                       // O(1)
    }

    // 'tieneHijoIzquierdo' cuesta O(1).
    private boolean tieneHijoIzquierdo(int indice) {
        return indice >= 0 && indice * 2 + 1 < this.arrayCiudad.length;                        // O(1)
    }
